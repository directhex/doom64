...
{
	{	"SFONT",	"EPJX"	},{	"STATUS",	"EPJX"	},{	"SPACE",	"EPJX"	},{	"MOUNTA",	"EPJX"	},
	{	"MOUNTB",	"EPJX"	},{	"MOUNTC",	"EPJX"	},{	"JPMSG01",	"PJ"	},{	"JPMSG02",	"PJ"	},
...
void Sprite_Convert(int lump)
{
	...
	if(palID[0] & 0x80) palID[0] = palID[0] - 0x80;

	if(palID[0] == 'P' &&
		palID[1] == 'A' &&
		palID[2] == 'L')
	{
		Sprite_GetPaletteLump(d64PaletteLump[extPalLumpCount].extPalLumps, lump);
		strncpy(d64PaletteLump[extPalLumpCount].name, romWadFile.lump[lump].name, 8);
	...
	exs->data = (byte*)Mem_Alloc(sizeof(byte)*(rs->width * rs->height) + 
		(rs->compressed == -1 ? 0 : (sizeof(dPalette_t)*256)));
	...
	else
	{
		...
		exs->size = rs->compressed == -1 ? (rs->width*rs->height) : ((rs->width*rs->height)/2);
	}
...
void Sprite_GetPalette(d64RawSprite_t* d64Spr, dPalette_t* palette, int lump)
{
	...
	data = (word*)(romWadFile.lumpcache[lump] + (d64Spr->compressed == -1 ? 
		(d64Spr->width*d64Spr->height) : (d64Spr->cmpsize)) + sizeof(d64RawSprite_t));
	WGen_ConvertN64Pal(palette, data, d64Spr->compressed == -1 ? 256 : CMPPALCOUNT);
}
...
void Sprite_Setup(void)
{
	...
	if(RomFile.header.CountryID != 'E')
	{
		...
		{
			if(!Rom_VerifyRomCode(&BloodSprites[i])) continue;
	...
	{
		if(!Rom_VerifyRomCode(&HudSprNames[i])) continue;
