...
bool __stdcall LoadingDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch(uMsg)
    {
    case WM_INITDIALOG:
...
int main(int argc, char* argv[])
{
    ...
    if(argc < 2)
    {
        WGen_Printf("Provide the location of the Doom64 Rom as a command line parameter.");
        return 0;
    }
...
#ifdef _WIN32
    if(hwndWait) EndDialog(hwndWait, FALSE);
#endif
