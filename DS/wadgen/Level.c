...
void Level_GetMapWad(int num)
{
    ...
    romWadFile.lumpcache[lump] = Wad_GetLump(romWadFile.lump[lump].name, true);
    ...
    memcpy(buffer+sizeof(wadheader_t), romWadFile.lumpcache[lump]+sizeof(wadheader_t),
        (wad.lmpdirpos - sizeof(wadheader_t)) + (sizeof(lump_t)*wad.lmpcount));

#ifdef HASHINDICES

    l = (lump_t*)(buffer+wad.lmpdirpos);
    Level_HashSidedefs(buffer + l[ML_SIDEDEFS].filepos, l[ML_SIDEDEFS].size);
