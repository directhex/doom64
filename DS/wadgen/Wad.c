...
// $Author: svkaiser $
// $Revision: 1097 $
// $Date: 2012-04-01 17:24:04 -0500 (Sun, 01 Apr 2012) $
//
...
void Wad_AddOutputSprite(d64ExSpriteLump_t* sprite)
{
    ...
    else if(sprite->sprite.useExtPal && sprite->lumpRef > Wad_GetLumpNum("RECTO0"))
    {
        ...
        for(i = 0; i < 256; i++)
            dspal[i] = RGBDS(pngpal[i].r, pngpal[i].g, pngpal[i].b);

        memcpy((data+pos), dspal, sizeof(short)*256);
    }
...
void Wad_AddOutputPalette(d64PaletteLump_t* palette)
{
	...
	for(i = 0; i < 256; i++)
        dspal[i] = RGBDS(
        palette->extPalLumps[i].r,
        palette->extPalLumps[i].g,
        palette->extPalLumps[i].b
        );

	Wad_AddOutputLump(name, size, (byte*)dspal);
}
