...
{
	{	"SYMBOLS",	"EPJX"	},{	"USLEGAL",	"EPJX"	},{	"TITLE",	"EPJX"	},{	"EVIL",		"EPJX"	},
	{	"FIRE",		"EPJX"	},{	"CLOUD",	"EPJX"	},{	"IDCRED1",	"EPJX"	},{	"IDCRED2",	"EPJX"	},
	{	"WMSCRED1",	"EPJX"	},{	"WMSCRED2",	"EPJX"	},{	"FINAL",	"EPJX"	},{	"JPCPAK",	"J",	},
...
void Gfx_CreateEXLump(gfxEx_t* pcGfx, gfxRom_t* romGfx)
{
	...
	for(h = 0; h < romGfx->header.height; h++)
	{
		for(w = 0; w < romGfx->header.width; w++)
			pcGfx->data[(h * pcGfx->width) + w] = romGfx->data[(h * romGfx->header.width) + w];
	}
...
void Gfx_Setup(void)
{
	...
	{
		if(!Rom_VerifyRomCode(&GfxNames[i])) continue;
		...
		else
		{
            ...
            if(!strncmp("FIRE", GfxNames[i].name, 8))
                Gfx_CreateRomFireLump(&gfxRom[i], romWadFile.lumpcache[l]);
            else
			    Gfx_CreateRomLump(&gfxRom[i], romWadFile.lumpcache[l]);
