...
void* Wad_GetLump(char* name, bool dcmpType) 
{
	...
	memcpy(data, romWadData + romWadFile.lump[lump].filepos, romWadFile.lump[lump].size);
	...
	if(data == NULL) 
	{
		WGen_Complain("Wad_GetLump: Couldn't allocate %u bytes from %s\n",
...
static uint Wad_FindIwadLocation(void)
{
	...
	WGen_Complain("Wad_FindIwadLocation: Unable to locate IWAD. Possible invalid ROM..");
	return 0;
}
...
void Wad_GetIwad(void)
{
	...
	romWadFile.size = ((romWadFile.header.lmpcount*sizeof(lump_t)) + romWadFile.header.lmpdirpos);
	memcpy(&romWadFile.lump, romWadData + romWadFile.header.lmpdirpos, romWadFile.header.lmpcount*sizeof(lump_t));
}
...
void Wad_AddOutputSprite(d64ExSpriteLump_t* sprite)
{
	...
	if(sprite->sprite.useExtPal)
	{
		...
		for(j = 0; j < extPalLumpCount; j++)
		{
			...
			if(!strncmp(name, (char*)(d64PaletteLump[j].name + 3), 4))
			{
				ok = true;
				memcpy(pngpal, d64PaletteLump[j].extPalLumps, sizeof(dPalette_t) * 256);
				break;
			}
		}
	...
	png = Png_Create(
		...
		sprite->sprite.useExtPal ? 16 : 1,	// 256 colors if external
		sprite->sprite.useExtPal ? pngpal : sprite->palette,
		sprite->sprite.useExtPal ? 8 : 4,	// 8 bit if external
		sprite->data,
		sprite->lumpRef,
		&pngsize);
...
void Wad_AddOutputTexture(d64ExTexture_t* tex)
{
	...
	png = Png_Create(
		...
		tex->header.numpal,
		pngpal,
		4,	// texture bytes are always packed so its always 4 bits
		tex->data,
		tex->lumpRef,
		&pngsize);
