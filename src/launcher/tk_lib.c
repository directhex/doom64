	...
	switch(def->type)
	{
		...
		case 's':	//find a quoted string
			...
			*def->strValue = dataStrToken[tkStrSlot++];
			break;
		default:
			L_Complain("tk_ProcessDefs: Unknown type: %c", def->type);
			break;
	}
}
...
void tk_getTokenNum(void) 
{
	...
	do 
	{
		...
		else 
		{
			...
			{
				if(parse[tkPos] > 0x20) 
					L_Complain("getTokenNum Line %i: Not a number..\n", tkLine);
			}
		}
	}	while(tkPos++ != lastByte);
}
...
void tk_getTokenStr(void) 
{
	...
	do 
	{
		...
		if(parse[tkPos] == '"') 
		{
			...
			{
				if(parse[tkPos] == '\n') 
					L_Complain("getTokenStr Line %i: Newline in string..\n", tkLine);

				if(tkPos >= lastByte)
					L_Complain("getTokenStr Line %i: End of file before completing string..\n", tkLine);

				dataStrToken[tkStrSlot][i++] = parse[tkPos];
			} while(parse[++tkPos] != '"');
			return;
		}
		else if(!(parse[tkPos] <= 0x20)) L_Complain("getTokenStr Line %i: String does not begin with quotes..\n");
	} while(tkPos++ != lastByte);
}
